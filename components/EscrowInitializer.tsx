...
import { InitializePayload, TransactionResult, SendTransactionResponse } from '@/types/trustless-work'
  ...
  const handleInitializeEscrow = async () => {
    ...
    try {
      ...
      const payload: InitializePayload = {
        ...
        deadline: new Date(Date.now() + assetConfig.defaultEscrowDeadlineDays * 24 * 60 * 60 * 1000).toISOString(), // Configurable days from now
        metadata: {
      ...
      const sendResult: SendTransactionResponse = await sendTransaction(signedTransaction.signedTxXdr)
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="bg-gray-50 p-4 rounded-md mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
          <div><span className="font-medium">Asset:</span> {assetConfig.defaultAsset.code}</div>
          <div><span className="font-medium">Issuer:</span> {assetConfig.defaultAsset.issuer.slice(0, 20)}...</div>
          <div><span className="font-medium">Decimals:</span> {assetConfig.defaultAsset.decimals.toLocaleString()}</div>
          <div><span className="font-medium">Platform Fee:</span> {assetConfig.platformFee}%</div>
          <div><span className="font-medium">Release Mode:</span> Multi-release</div>
          <div><span className="font-medium">Wallet Address:</span> {walletData?.publicKey?.slice(0, 8)}...{walletData?.publicKey?.slice(-8)}</div>
        </div>
      </div>
      ...
      {/* Transaction Result */}
      {transactionResult && (
        <div className={`border rounded-md p-4 ${transactionResult.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          {transactionResult.success ? (
            <div>
              <h3 className="font-semibold text-green-800 mb-2">Escrow Initialized Successfully!</h3>
