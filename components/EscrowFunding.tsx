    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="bg-blue-50 p-4 rounded-md mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm text-blue-700">
          ...
          <div><span className="font-medium">Total Amount:</span> {formatAmount(totalAmount, escrowData.escrow.asset.decimals)} {escrowData.escrow.asset.code}</div>
      ...
      <div className="bg-gray-50 p-4 rounded-md mb-6">
        ...
        <div className="space-y-2">
          {escrowData.escrow.releases.map((release, index) => (
            <div key={release.id} className="flex justify-between items-center bg-white p-2 rounded border">
              <span className="text-sm font-medium">Milestone {index + 1}:</span>
              <span className="text-sm text-gray-600">
                {formatAmount(parseInt(release.amount), escrowData.escrow.asset.decimals)} {escrowData.escrow.asset.code}
              </span>
            </div>
          ))}
        </div>
      ...
      {/* Funding Result */}
      {fundingResult && (
        <div className={`border rounded-md p-4 ${fundingResult.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          {fundingResult.success ? (
            <div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-green-700">
                  <span className="font-medium">Amount Funded:</span> {formatAmount(totalAmount, escrowData.escrow.asset.decimals)} {escrowData.escrow.asset.code}
                </p>
              </div>
            </div>
          ) : (
