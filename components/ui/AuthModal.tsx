...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, mode }) => {
  ...
  const usernameSuggestions = [
    ...
    'CryptoExplorer', 'BlockchainNinja', 'StellarWarrior', 'DeFiMaster', 'Web3Wizard',
    'NexusGuardian', 'TrustlessHero', 'EscrowExpert', 'CryptoVoyager', 'StellarPilot',
    'DeFiAdventurer', 'BlockchainPioneer', 'Web3Explorer', 'CryptoNavigator', 'StellarSage',
    'TrustlessTrailblazer', 'EscrowEnthusiast', 'DeFiDynamo', 'CryptoChampion', 'StellarStar',
    
    // Tech & Innovation
    'CodeCrusader', 'TechTitan', 'DigitalDragon', 'CyberSage', 'QuantumQuasar',
    'NeuralNexus', 'DataDynamo', 'CloudCommander', 'ByteBender', 'PixelPioneer',
    'AlgorithmAce', 'BinaryBard', 'CircuitSage', 'LogicLegend', 'SyntaxSorcerer',
    'FrameworkFalcon', 'ProtocolPhoenix', 'SystemSage', 'NetworkNinja', 'InterfaceIcon',
    
    // Stellar & Space Theme
    'StellarSovereign', 'CosmicCoder', 'GalaxyGuardian', 'NebulaNavigator', 'OrbitOracle',
    'AsteroidAce', 'CometCommander', 'MeteorMaster', 'PlanetPilot', 'SolarSage',
    'LunarLegend', 'MarsMariner', 'JupiterJuggernaut', 'SaturnSage', 'UranusUnicorn',
    'NeptuneNavigator', 'PlutoPioneer', 'VenusVoyager', 'MercuryMaster', 'EarthExplorer',
    
    // DeFi & Finance
    'YieldYogi', 'LiquidityLord', 'SwapSage', 'PoolPioneer', 'FarmFalcon',
    'StakeSovereign', 'BorrowBard', 'LendLegend', 'MintMaster', 'BurnBender',
    'TokenTitan', 'CoinCommander', 'AssetAce', 'PortfolioPhoenix', 'TradeTrailblazer',
    'MarketMariner', 'PricePilot', 'VolumeVoyager', 'SpreadSage', 'MarginMaster',
    
    // Gaming & Adventure
    'QuestQuasar', 'DungeonDragon', 'LootLegend', 'RaidRanger', 'GuildGuardian',
    'ClanCommander', 'BattleBard', 'WarriorWizard', 'KnightNinja', 'PaladinPilot',
    'MageMariner', 'RogueRanger', 'BardBender', 'ClericCrusader', 'MonkMaster',
    'BarbarianBard', 'SorcererSage', 'WarlockWizard', 'DruidDynamo', 'RangerRocket',
    
    // Nature & Elements
    'FireFalcon', 'WaterWizard', 'EarthExplorer', 'AirAce', 'LightningLegend',
    'ThunderTitan', 'StormSage', 'WindWanderer', 'IceIcon', 'FlameFighter',
    'ForestFalcon', 'MountainMaster', 'RiverRanger', 'OceanOracle', 'DesertDragon',
    'ValleyVoyager', 'CanyonCommander', 'PeakPioneer', 'SummitSage', 'CliffCrusader',
    
    // Mystical & Magic
    'CrystalCrusader', 'EnchantmentExpert', 'SpellSage', 'RuneRanger', 'GlyphGuardian',
    'MysticMariner', 'ArcaneAce', 'EtherealExplorer', 'CelestialSage', 'DivineDragon',
    'SacredSorcerer', 'BlessedBard', 'CursedCommander', 'HexedHero', 'CharmedChampion',
    'EnchantedEagle', 'MagicalMage', 'WitchWizard', 'WarlockWarrior', 'SorcererSage',
    
    // Modern & Urban
    'CyberSage', 'NeonNinja', 'PixelPioneer', 'DigitalDragon', 'VirtualVoyager',
    'HologramHero', 'MatrixMaster', 'GridGuardian', 'NodeNavigator', 'LinkLegend',
    'StreamSage', 'CloudCommander', 'ServerSorcerer', 'DatabaseDragon', 'CacheCrusader',
    'APIArchitect', 'SDKSage', 'FrameworkFalcon', 'LibraryLegend', 'ModuleMaster',
    
    // Creative & Artistic
    'ArtisanAce', 'CreatorCrusader', 'DesignDragon', 'CanvasCommander', 'PalettePilot',
    'BrushBard', 'SketchSage', 'DoodleDynamo', 'PaintPioneer', 'InkIcon',
    'PixelPainter', 'VectorVoyager', 'RasterRanger', 'GraphicGuardian', 'LogoLegend',
    'BrandBard', 'IdentityIcon', 'StyleSage', 'AestheticAce', 'VisualVoyager',
    
    // Professional & Business
    'ExecutiveExplorer', 'ManagerMaster', 'DirectorDragon', 'CEOCommander', 'CTOCrusader',
    'CFOChief', 'COOCaptain', 'VPVictor', 'DirectorDynamo', 'LeadLegend',
    'SeniorSage', 'PrincipalPilot', 'ArchitectAce', 'EngineerEagle', 'DeveloperDragon',
    'AnalystAce', 'ConsultantCrusader', 'AdvisorArchitect', 'StrategistSage', 'PlannerPioneer'
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentMode === 'signup') {
        await signUp(walletData.publicKey, formData.username || undefined, formData.email || undefined);
      } else {
            ...
            {currentMode === 'signup' && (
              <div className="bg-white/5 rounded-lg border border-white/10 overflow-hidden">
                ...
                >
                  ...
                  <span className={`transform transition-transform duration-200 ${isBenefitsExpanded ? 'rotate-180' : 'rotate-0'}`}>
                    ▼
                  </span>
                </button>
                {isBenefitsExpanded && (
          ...
          <div className="px-6 py-4 bg-white/5 border-t border-white/10">
            <div className="text-center">
              <p className="text-white/60 text-xs">
                By {currentMode === 'signup' ? 'creating an account' : 'signing in'}, you agree to our terms of service
              </p>
