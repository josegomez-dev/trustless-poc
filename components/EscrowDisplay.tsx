    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div><span className="font-medium">Status:</span> 
              <span className={`ml-2 px-2 py-1 rounded text-xs ${
                escrowData.status === 'success' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
              }`}>
                {escrowData.status}
              </span>
            </div>
            {escrowData.message && (
              <div><span className="font-medium">Message:</span> {escrowData.message}</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-green-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div><span className="font-medium">Issuer:</span> {formatAddress(escrowData.escrow.asset.issuer)}</div>
            <div><span className="font-medium">Decimals:</span> {escrowData.escrow.asset.decimals.toLocaleString()}</div>
            <div><span className="font-medium">Amount:</span> {formatAmount(escrowData.escrow.amount, escrowData.escrow.asset.decimals)} {escrowData.escrow.asset.code}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="bg-purple-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <div><span className="font-medium">Buyer:</span> {formatAddress(escrowData.escrow.buyer)}</div>
            <div><span className="font-medium">Seller:</span> {formatAddress(escrowData.escrow.seller)}</div>
            <div><span className="font-medium">Arbiter:</span> {formatAddress(escrowData.escrow.arbiter)}</div>
          </div>
        </div>
        ...
        <div className="bg-orange-50 p-4 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            <div><span className="font-medium">Platform Fee:</span> {escrowData.escrow.platformFee}%</div>
            <div><span className="font-medium">Deadline:</span> {formatDate(escrowData.escrow.deadline)}</div>
            <div><span className="font-medium">Terms:</span> {escrowData.escrow.terms}</div>
          </div>
        </div>
      </div>
            ...
            <div key={release.id} className="bg-white p-3 rounded border">
              <div className="flex justify-between items-center">
                <div className="text-sm">
                  <span className="font-medium">Release {index + 1}:</span> {formatAmount(release.amount, escrowData.escrow.asset.decimals)} {escrowData.escrow.asset.code}
                </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div><span className="font-medium">Created:</span> {formatDate(escrowData.escrow.createdAt)}</div>
          <div><span className="font-medium">Updated:</span> {formatDate(escrowData.escrow.updatedAt)}</div>
        </div>
      </div>
